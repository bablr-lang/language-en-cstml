...
export const grammar = class CSTMLGrammar {
  constructor() {
    ...
    this.attributes = new Map(
      Object.entries({
        InitializerTag: { isArray: undefined },
  ...
  *Tag() {
    ...
    } else if (yield eatMatch(m`<InitializerTag /\[\]|undefined/ />`)) {
  ...
  *InitializerTag() {
    let arr;
    if ((arr = yield eatMatch(m`sigilToken: <* '[]' />`))) {
  ...
  *PropertyValue({ props: { allowFragment } }) {
    if (yield eatMatch(m`<NullTag 'null' />`)) {
      ...
      yield eat(m`<Node /\<[^#@]|['"]/ />`, o({ allowFragment, propertyValue: true }));
    }
  }
  ...
  *OpenNodeTag({
    ...
    if (isFragment) {
      ...
      type = yield eatMatch(m`type$: <Identifier />`, o({}), o({ bind: true }));
    }
  ...
  *Identifier() {
    let q;
    q = yield eatMatch(
      m`openToken: <* ${buildString('`')} { balanced: ${buildString('`')} } />`,
      o({}),
      o({ bind: true }),
    );
    ...
    if (q) {
      yield eat(
        m`closeToken: <* ${buildString('`')} { balancer: true } />`,
        o({}),
        o({ bind: true }),
      );
    } else {
