...
export default class CSTML {
  ...
  constructor() {
    ...
    this.attributes = BMap.from(
      ...
      entry('NodeFlags', freezeRecord({ token: undefined, hasGap: undefined })),
    );
  }
  ...
  *SumsTag() {
    yield eat(m`openToken*: <* '#[' />`);
    yield* eatMatchTrivia();
    yield eat(m`refsSums$: <RefsSums />`);
    yield* eatMatchTrivia();
    yield eat(m`#separatorToken: <* ',' />`);
    yield* eatMatchTrivia();
    yield eat(m`gearHash$: :JSON: <String />`);
    yield* eatMatchTrivia();
    yield eat(m`#separatorToken: <* ',' />`);
    yield* eatMatchTrivia();
    yield eat(m`lineBreaks$: :JSON: <Number />`);
    yield* eatMatchTrivia();
    yield eat(m`#separatorToken: <* ',' />`);
    yield* eatMatchTrivia();
    yield eat(m`gaps$: :JSON: <Number />`);
    yield* eatMatchTrivia();
    yield eat(m`closeToken*: <* '#]' />`);
  }
  ...
  *RefSums() {
    yield eatMatch(m`ref: <ReferenceTag />`);
    yield* eatMatchTrivia();
    yield eat(m`count: :JSON: <Number />`);

    yield* eatMatchTrivia();
    yield eat(m`#separatorToken: <* ',' />`);
    yield* eatMatchTrivia();

    let value = yield eat(m`isArray: :JSON: <Number />`);

    if (value !== 1 && value !== 0) yield fail();
  }
