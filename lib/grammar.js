 ...
 * @generatedfrom ./grammar.macro.js#67d6f7b604b93745a981a7a5d4a1b7739185efc7
...
export const grammar = class CSTMLGrammar {
  ...
  *Tag() {
    yield eat(m`<_Any />`, [m`<NullTag 'null' />`, m`<InitializerTag /\[\]|undefined/ />`, m`<AttributeDefinition '{' />`, m`<ReferenceTag /[.#@a-zA-Z\u0060\u{80}-\u{10ffff}]/ />`, m`<BindingTag ':' />`, m`<LiteralTag /['"]/ />`, m`<DoctypeTag '<!' />`, m`<GapTag '<//>' />`, m`<ShiftTag '^^^' />`, m`<CloseNodeTag '</' />`, m`<OpenNodeTag '<' />`]);
  }
  *Document() {
  ...
  *ReferenceTag({
    ...
    yield* eatMatchTrivia();
    if (yield eatMatch(m`openIndexToken: <* '[' { balanced: ']' } />`, o({}), o({
      bind: true
    }))) {
      yield* eatMatchTrivia();
  ...
  *ReferenceFlags() {
    yield eatMatch(m`expressionToken: <* '+' />`, o({}), o({
      bind: true
    }));
    yield eatMatch(m`hasGapToken: <* '$' />`, o({}), o({
      bind: true
    }));
  }
  *BindingTag() {
  ...
  *InitializerTag() {
    let arr;
    if (arr = yield eatMatch(m`sigilToken: <* '[]' />`)) {} else if (yield eat(m`sigilToken: <*Keyword 'undefined' />`)) {}
  ...
  *NodeChild({
    ...
    if (token) {
      yield eat(m`<_Any />`, [m`<AttributeDefinition '{' />`, m`<Property '@' />`, m`<LiteralTag /['"]/ />`]);
    } else {
  ...
  *PropertyValue({
    ...
    yield eat(m`<_Any />`, [m`<NullTag 'null' />`, m`<GapTag '<//>' />`, m`<InitializerTag /\[\]|undefined/ />`, [m`<Node /\<[^#@]|['"]/ />`, o({
      allowFragment,
      propertyValue: true
    })]]);
  }
  *NodeFlags() {
    yield eatMatch(m`tokenToken: <* '*' />`, o({}), o({
      bind: true
    }));
    yield eatMatch(m`hasGapToken: <* '$' />`, o({}), o({
      bind: true
    }));
    yield eatMatch(m`fragmentToken: <* '_' />`, o({}), o({
      bind: true
    }));
    yield eatMatch(m`multiFragmentToken: <* '_' />`, o({}), o({
      bind: true
    }));
  }
  *OpenNodeTag({
    ...
    if (isFragment) {
      ...
      type = yield eatMatch(m`type$: <Identifier />`, o({}), o({
        bind: true
      }));
    }
    ...
    let sc;
    if (!isFragment) {
      sc = yield eatMatch(m`selfClosingToken: <* '/' />`, o({}), o({
        bind: true
      }));
    } else {
  ...
  *Identifier() {
    let q;
    q = yield eatMatch(m`openToken: <* ${buildString('`')} { balanced: ${buildString('`')} } />`, o({}), o({
      bind: true
    }));
    ...
    if (q) {
      yield eat(m`closeToken: <* ${buildString('`')} { balancer: true } />`, o({}), o({
        bind: true
      }));
    } else {
      yield eat(m`closeToken: null`);
  ...
  *EscapeCode() {
    ...
    if (yield eatMatch(m`openToken: <* '{' { balanced: '}' } />`, o({}), o({
      bind: true
    }))) {
