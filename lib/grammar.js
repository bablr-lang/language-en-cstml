 ...
 * @generatedfrom ./grammar.macro.js#bf0cd6a17e8c42d0fe5d6e4488626b4f65a09171
...
export const grammar = class CSTMLGrammar {
  ...
  *Tag() {
    yield eat(m`<Any />`, [m`<NullTag 'null' />`, m`<InitializerTag '[]' />`, m`<ReferenceTag /[.#@a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`, m`<LiteralTag /['"]/ />`, m`<DoctypeTag '<!' />`, m`<GapTag '<//>' />`, m`<ShiftTag '^^^' />`, m`<CloseNodeTag '</' />`, m`<OpenNodeTag '<' />`]);
  }
  *Document() {
  ...
  *DoctypeTag() {
    ...
    yield* eatMatchTrivia();
    yield eatMatch(m`attributes$: <JSON:Object />`, null, o({
      bind: true
    }));
  ...
  *ReferenceTag({
    ...
    yield* eatMatchTrivia();
    if (yield eatMatch(m`openIndexToken: <*Punctuator '[' { balanced: ']' } />`, null, o({
      bind: true
    }))) {
      yield* eatMatchTrivia();
  ...
  *ReferenceFlags() {
    yield eatMatch(m`expressionToken: <*Punctuator '+' />`, null, o({
      bind: true
    }));
    yield eatMatch(m`hasGapToken: <*Punctuator '$' />`, null, o({
      bind: true
    }));
  }
  *GapTag() {
  ...
  *NodeChild({
    ...
    if (token) {
      ...
      yield eat(m`<Any />`, [m`<Property /[.#\\a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`, m`<ShiftTag '^^^' />`]);
    }
  }
  *Property() {
  ...
  *PropertyValue() {
    yield eat(m`<Any />`, [m`<NullTag 'null' />`, m`<GapTag '<//>' />`, m`<InitializerTag '[]' />`, m`<Node /\<[^#@]/ />`]);
  }
  *NodeFlags() {
    yield eatMatch(m`tokenToken: <*Punctuator '*' />`, null, o({
      bind: true
    }));
    yield eatMatch(m`hasGapToken: <*Punctuator '$' />`, null, o({
      bind: true
  ...
  *OpenNodeTag({
    ...
    let sc = yield eatMatch(m`selfClosingTagToken: <*Punctuator '/' />`, null, o({
      bind: true
    }));
  ...
  *Language() {
    yield eat(m`<Any />`, [m`<JSON:String /['"]/ />`, m`<IdentifierPath /[\\a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`]);
  }
  *IdentifierPath() {
  ...
  *Identifier() {
    let q;
    q = yield eatMatch(m`openToken: <*Punctuator ${buildString('`')} { balanced: ${buildString('`')} } />`, null, o({
      bind: true
    }));
    ...
    if (q) {
      yield eat(m`closeToken: <*Punctuator ${buildString('`')} { balancer: true } />`, null, o({
        bind: true
      }));
    } else {
