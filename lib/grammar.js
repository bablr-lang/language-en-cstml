...
export default class CSTML {
  ...
  *TreeNode({ props }) {
    if (yield match(m`/['"]/`)) {
      ...
      yield eat(m`closeTag*: <CloseTag />`, o({}), o({ literal: true, allowEmpty: true }));
  ...
  *Node({ s }) {
    if (s().shifted) {
      ...
      } else if (yield eatMatch(m`<CloseTag '</>' />`)) {
        yield fail();
      } else {
        yield eat(m`<TreeNode />`);
      }
