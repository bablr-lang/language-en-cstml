...
export const grammar = class CSTMLGrammar {
  constructor() {
    ...
    this.attributes = new Map(
      Object.entries({
        ...
        ReferenceTag: { isArray: undefined },
        NodeFlags: { token: undefined, hasGap: undefined, fragment: undefined, cover: undefined },
      }),
    );
  }
  ...
  *Tag({ props: { kind } }) {
    if (yield eatMatch(m`<HashTag '##' />`)) {
    }

    if (kind !== 'value') {
      if (yield eatMatch(m`<AttributeDefinition '{' />`)) {
      } else if (yield eatMatch(m`<ReferenceTag /[.#@_a-zA-Z\u0060\u{80}-\u{10ffff}]/ />`)) {
      ...
      } else if (yield eatMatch(m`<OpenNodeTag /\<|['"]/ />)`)) {
      }
    }
    if (kind !== 'key') {
      ...
      } else if (yield eat(m`<OpenNodeTag /\<|['"]/ />`)) {
      }
    }
  }
  ...
  *Document() {
    ...
    yield* eatMatchTrivia();
    yield eat(m`tree$: <_Expression />`, o({ forceFragment: true }));
  }
  ...
  *ReferenceTag() {
    ...
    let isArray = false;
    if (yield eatMatch(m`openIndexToken*: <* '[' />`)) {
      isArray = true;
      yield* eatMatchTrivia();
      yield eat(m`closeIndexToken*: <* ']' />`);
    }

    yield defineAttribute('isArray', isArray);
  ...
  *ReferenceFlags() {
    ...
    yield eatMatch(m`intrinsicToken*: <* '*' />`);
    yield eatMatch(m`hasGapToken*: <* '$' />`);
  }
  ...
  *Property({ s }) {
    if (s().held) {
      yield eat(m`referenceTag$: <ShiftTag />`);
    } else {
  ...
  *ShiftExpression() {
    yield eat(m`original+$: <BoundNode />`);
    yield* eatMatchTrivia();
  ...
  *NodeFlags({ props: { token } }) {
    ...
    let frag = yield eatMatch(m`fragmentToken*: <* '_' />`);
    let multiFrag = yield eatMatch(m`multiFragmentToken*: <* '_' />`);

    yield defineAttribute('token', !!(token_ || token));
    yield defineAttribute('hasGap', !!hasGap);
    yield defineAttribute('fragment', !!frag);
    yield defineAttribute('cover', !!frag && !multiFrag);
  }

  *OpenNodeTag({ props: { forceFragment = false } }) {
    if (yield match(re`/['"]/`)) {
    ...
    let isFragment = multiFragmentFlag;
