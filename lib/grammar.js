...
export const grammar = class CSTMLGrammar {
  ...
  *ReferenceFlags() {
    ...
    yield eatMatch(m`intrinsicToken*: <* '*' />`, o({}), o({ bind: true }));
  }
  ...
  *OpenNodeTag({ s, props: { forceFragment = false, propertyValue = false } }) {
    ...
    let multiFragmentFlag = !!flags.node.get('multiFragmentToken');
  ...
  *Identifier() {
    let q;
    q = yield eatMatch(
      (idOpenMatcher ??= m`openToken*: <* ${buildString('`')} { balanced: ${buildString(
        '`',
      )} } />`),
      null,
      o({ bind: true }),
    ...
    if (q) {
      yield eat(
        (idCloseMatcher ??= m`closeToken*: <* ${buildString('`')} { balancer: true } />`),
        null,
        o({ bind: true }),
      );
    } else {
