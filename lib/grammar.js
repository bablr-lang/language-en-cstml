...
export const grammar = triviaEnhancer(
  ...
  class CSTMLGrammar {
    ...
    *EscapeSequence(props, s, ctx) {
      ...
      let code = ctx.getSourceText(yield i`eat(<*EscapeCode> 'code')`);
      yield i`bindAttribute(cooked ${buildString(escapables.get(code))})`;
    }

    @Node
    *EscapeCode(props, { span }) {
      span === 'String:Single' ? yield i`eat(/[\\/nrt0']/)` : yield i`eat(/[\\/nrt0"]/)`;
    }
