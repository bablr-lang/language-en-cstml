...
export const grammar = class CSTMLGrammar {
  *[Symbol.for('@bablr/fragment')]({ props: { productionName } }) {
  ...
  *Tag() {
    yield eat(m`<Any />`, [
      m`<NullTag 'null' />`,
      m`<InitializerTag '[]' />`,
      m`<ReferenceTag /[.#@a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`,
      m`<LiteralTag /['"]/ />`,
      m`<DoctypeTag '<!' />`,
      m`<GapTag '<//>' />`,
      m`<ShiftTag '^^^' />`,
  ...
  *NodeChild({ props: { token } }) {
    if (token) {
      ...
      yield eat(m`<Any />`, [
        m`<Property /[.#\\a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`,
        m`<ShiftTag '^^^' />`,
      ]);
    }
  }
  ...
  *Language() {
    yield eat(m`<Any />`, [
      m`<JSON:String /['"]/ />`,
      m`<IdentifierPath /[\\a-zA-Z_\u0060\u{80}-\u{10ffff}]/ />`,
    ]);
  }
  ...
  *IdentifierContent({ props: { quoted = false } }) {
    ...
    do {
      if ((esc = yield match('\\'))) {
        ...
        if (!quoted) {
          lit = yield eatMatch(re`/[a-zA-Z_\u{80}-\u{10ffff}][a-zA-Z0-9_\u{80}-\u{10ffff}-]*/`);
        } else {
  ...
  *EscapeCode() {
    if (yield eatMatch(m`typeToken: <*Keyword 'u' />`)) {
      if (
        yield eatMatch(m`openToken: <*Punctuator '{' { balanced: '}' } />`, null, o({ bind: true }))
      ) {
