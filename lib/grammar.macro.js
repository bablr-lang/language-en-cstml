...
export const grammar = class CSTMLGrammar {
  ...
  *Node({ props }) {
    ...
    const selfClosingToken = open.get('selfClosingToken');
    const token = !!flags?.get('tokenToken');
    const selfClosing = !!selfClosingToken;
  ...
  *Property({ ctx }) {
    ...
    yield eat(
      ...
      o({ allowFragment: ctx.sourceTextFor(ref.get('type')) === '_' }),
    );
  }
  ...
  *EscapeSequence({ ctx, state: { span } }) {
    ...
    if (first(codeNode.tags)?.value.tags[0].value.flags.token) {
